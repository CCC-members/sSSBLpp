...
for gen = 1:Ngen
    ...
    for band = 1:Nbands
        [P,p]          = corr([j1(:,band) j2(:,band) j1neigh(:,band) j2neigh(:,band)],'Type','Spearman','Tail','right');
        P              = P(1,2);
        p              = p(1,2);
        cor(gen,band)  = P;
        pvc(gen,band)  = p;
        [P,p]          = partialcorr([j1(:,band) j2(:,band) j1neigh(:,band) j2neigh(:,band)],'Type','Spearman','Tail','right');
        P              = P(1,2);
        p              = p(1,2);
        pcr(gen,band)  = P;
        pvp(gen,band)  = p;
