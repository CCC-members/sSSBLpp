function [s2j,sigma2j_post,Tjv,Svv,Lvj,scaleJ,scaleLvj] = sSSBLpp_ultralarge(Svv,Lvj,m,parcellation)
...
for count_gen = 1:q
    sigma2j_post(count_gen) = sigma2j(count_gen)-sigma2j_post0(count_gen,:)*Lvjsigma2j(:,count_gen);
end
...
scaleJ        = mean(abs(s2j))/max(sigma2j_post);
Svv           = Svv/scaleJ;
...
for cont1 = 1:maxiter_outer
    ...
    for cont11 = 1:maxiter_inner
        ...
        for area = 1:a
            ...
            etha(idx_area)          = sqrt((1./4).^2+(alpha*rho*m).*sum(s2j(idx_area) + sigma2j_post(idx_area)))-1./4;
        end
    ...
    alpha      = (length(idx_alpha)/2 + s_alpha)/(m*sum((s2j(idx_alpha) + sigma2j_post(idx_alpha))./(sigma2j_bar(idx_alpha))) + r_alpha);
    %% Update rho
