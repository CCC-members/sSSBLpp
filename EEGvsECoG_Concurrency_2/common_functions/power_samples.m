...
for band = 1:Nbands
    ...
    % source 1
    J01                  = J0{band}(:,1);
    data1                = sim_data.functional.data1.Data_TS{band}; %Data_FC for band power and Data_TS{band} for filtered time series
    F1                   = sim_data.functional.data1.F;
    J1(:,:,band)         = band_power(data1,J01,T1,bands,band,F1,ind,isfield);
    % source 2
    J02                  = J0{band}(:,2);
